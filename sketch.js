...
//  p0 = createP("LIMMA_ASCENDANT")
  ...
  p1.position(320, 741);
  p3 = createP("Limma_ascendant uses cut-up real-time radio content from local stations — manipulates the fragments of audio and intersperses them throughout the performance space. The juxtaposition of the various stations colliding and being reassembled in different ways creates a constantly evolving reinterpretation of the sonic context of the material. Weather reports, traffic updates, local news, advertisements, and music channels are the sound sources for the project. Instead of using traditional radios and manually moving the dial to access channels, this project is based around a digital FM radio module which can access FM channels digitally, by scanning, programming, and storing the channels. The radio module is controlled via a Teensy v4.3 microcontroller, which algorithmically switches / glitches between channels to create anything from shifting nonsensical sequences to barely perceivable compositional sections. The microcontroller is also processing the audio from the radio module, using spatial / time-based effects such as panning, delays and reverbs, for further displacement of the original content.");
  p3.position(10, 785);
  // p2 = createP(time);
  // p2.position(10, 770);
